...
<script type="text/babel">
    ...
    const getClockWidgetApi = () => (
        window.ReactDashboardClockWidget && typeof window.ReactDashboardClockWidget === 'object'
            ? window.ReactDashboardClockWidget
            : null
    );

    const getClockWidgetDefaults = () => {
        const api = getClockWidgetApi();
        if (api && api.DEFAULTS && typeof api.DEFAULTS === 'object') {
            return { ...api.DEFAULTS, type: 'Clock' };
        }
        return { type: 'Clock', name: 'default', showHeader: true };
    };

    const sanitizeClockWidgetSettings = (raw) => {
        const api = getClockWidgetApi();
        if (api && typeof api.sanitizeSettings === 'function') {
            const sanitized = api.sanitizeSettings(raw);
            if (sanitized && typeof sanitized === 'object') {
                return { ...getClockWidgetDefaults(), ...sanitized, type: 'Clock' };
            }
        }
