...
class ReactDashboardApi extends CController {
    ...
    protected function doAction(): void {
        ...
        if ($action === 'get_groups') {
            ...
            $this->respond(array_values($groups));
            return;
        }

        if ($action === 'get_hosts_by_group') {
            $groupid = $this->getInput('groupid', null);
            $hosts = API::Host()->get([
            ...
            $this->respond(array_values($hosts));
            return;
        }
        ...
        if ($action === 'timestate_items') {
            ...
            $search = [];
            if ($item_key_search !== '') {
                $search['key_'] = self::normalizeSearchPattern($item_key_search);
            }
            if ($item_name_search !== '') {
                $search['name'] = self::normalizeSearchPattern($item_name_search);
            }
            if ($search !== []) {
            ...
            $this->respond(['items' => $result]);
            return;
        }

        if ($action === 'timestate_data') {
            $hostids = self::parseIds((string) $this->getInput('hostids_csv', ''));
            $itemids = self::parseIds((string) $this->getInput('itemids_csv', ''));
            if ($hostids === [] && $itemids === []) {
                $this->respond(['error' => 'Selecteer minstens een host of item.']);
                return;
            }

            $item_key_search = trim((string) $this->getInput('item_key_search', ''));
            $item_name_search = trim((string) $this->getInput('item_name_search', ''));
            $lookback_hours = self::clampInt((int) $this->getInput('lookback_hours', 24), 1, 24 * 31);
            $max_rows = self::clampInt((int) $this->getInput('max_rows', 20), 1, 200);
            $history_points = self::clampInt((int) $this->getInput('history_points', 500), 50, 5000);
            $row_sort = self::clampInt((int) $this->getInput('row_sort', 0), 0, 2);
            ...
            $value_map = self::parseStateMap($state_map_raw);
            ...
            foreach ($items as $item) {
                ...
                $segments = self::buildSegments($history, $time_from, $time_to, $value_map, $fallback_colors, $merge_equal_states);
                if ($segments === []) {
                    continue;
                }
        ...
        $this->respond(['error' => 'Onbekende action_type']);
    }

    private function respond($payload): void {
    ...
    private static function normalizeSearchPattern(string $value): string {
        $value = trim($value);
        if ($value === '') {
            return $value;
        }
    ...
    private static function parseStateMap(string $raw): array {
        $result = [];
    ...
    private static function resolveStateInfo(string $value, array $state_map, array $fallback_colors): array {
    ...
    private static function buildSegments(array $history, int $time_from, int $time_to, array $state_map, array $fallback_colors, bool $merge_equal): array {
        if ($history === []) {
            return [];
        }
        ...
        for ($i = 0; $i < $count; $i++) {
            ...
            $resolved = self::resolveStateInfo($raw_value, $state_map, $fallback_colors);
